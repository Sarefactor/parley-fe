<script lang="ts">
	...
	import { schema, type ConfirmationNodeOptions } from '$lib/stores/agent-schema-store.svelte';

	let { id }: NodeProps = $props();

	const config = $derived(schema.getNode(id));
	const options = $derived(config?.nodeOptions as ConfirmationNodeOptions | undefined);
</script>

<BaseNode nodeId={id} title="Confirmation Node">
