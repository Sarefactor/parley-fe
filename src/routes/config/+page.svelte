<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			loadError = e instanceof Error ? e.message : 'Failed to load agent schema.';
		} finally {
			loading = false;
		}
	});
...
<svelte:head>
	<title>Parley — Configure Agent Schema</title>
</svelte:head>
...
{#if confirmLeave}
	<Modal title="Unsaved Changes" onclose={() => (confirmLeave = false)}>
		<p class="confirm-text">
			You have unsaved changes. Abandon them and return to the Agent Schemas list?
		</p>
		<div class="confirm-actions">
			<button type="button" class="cancel-btn" onclick={() => (confirmLeave = false)}>
				Cancel
			</button>
			<button type="button" class="danger" onclick={() => void goto('/')}>
				Abandon Changes
			</button>
		</div>
	</Modal>
